Ext.define('Ext.ux.AwesomeCombo', {
    extend: 'Ext.form.ComboBox',
    alias:'widget.awesomecombo',
	/**
	...
	refreshDisplay: function(forced) {
		forced = forced || false;
		if (this.rendered === false || (forced === false && (this.is('expand') ||
			this.isSettingValue))) {
			return false;
		}
		...
		if (this.fireEvent('beforedisplayrefresh', this,
			...
			if (this.displayNb == 1) {
				...
				if (this.valueFound) {
					this.clearRawValue();
					this.el.removeCls(this.emptyClass);
	...
	updateEditState: function() {
		if (this.rendered) {
			if (this.readOnly) {
				...
				this.el.addCls('x-trigger-noedit');
				...
				if (!this.editable) {
					...
					this.el.removeCls('x-trigger-noedit');
				}
			}
		}
	},
	...
	initTrigger : function(){
		var ts = this.triggerWrap.select('.x-form-trigger', true),
		triggerField = this;
